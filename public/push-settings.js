...
export async function renderPushSettings(panel,api){
  ...
  const section=document.createElement('section');section.id='pushSettings';section.innerHTML='<hr><h2>iPhone notifications</h2><p class="muted">Optional, generic check-in reminders at the time saved above. Open Steady from your iPhone Home Screen to enable notifications. No health details appear in a notification.</p><p id="pushStatus" role="status">Checking availability…</p><div class="actions"><button type="button" class="ghost" id="enablePush">Enable on this iPhone</button><button type="button" class="ghost" id="testPush">Send test notification</button><button type="button" class="ghost" id="disablePush">Disable on this iPhone</button></div>';
  panel.append(section);
  const status=section.querySelector('#pushStatus'),enable=section.querySelector('#enablePush'),disable=section.querySelector('#disablePush'),test=section.querySelector('#testPush');
  let config;
  ...
  if(!config.available){status.textContent='Push delivery is not configured on the server yet. In-app reminders still work while Steady is open.';enable.disabled=true;disable.disabled=true;test.disabled=true;return}
  if(!supported){status.textContent='This browser does not support web push. On iPhone, launch Steady from its Home Screen icon.';enable.disabled=true;disable.disabled=true;test.disabled=true;return}
  const refresh=async()=>{const registration=await navigator.serviceWorker.ready,subscription=await registration.pushManager.getSubscription();status.textContent=subscription?'Enabled on this device. You can turn it off below.':'Off on this device. Enable only if you want gentle notifications.';enable.disabled=!!subscription;disable.disabled=!subscription;test.disabled=!subscription};
  try{await refresh()}catch{status.textContent='Push service is not ready. Reload Steady and try again.';enable.disabled=true;disable.disabled=true;test.disabled=true}
  ...
  test.onclick=async()=>{
    try{const registration=await navigator.serviceWorker.ready,subscription=await registration.pushManager.getSubscription();if(!subscription)return refresh();await api('/api/push/test',{method:'POST',body:JSON.stringify({endpoint:subscription.endpoint})});status.textContent='Test sent to the iPhone push service. Check for a Steady notification.'}catch(error){status.textContent=error.message||'Could not send a test notification.'}
  };
}
