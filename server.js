...
import { trainingTemplate, trainingContext, validSchedule } from './training.js';
...
async function route(req,res) {
  ...
  if(method==='GET' && u.pathname==='/manifest.webmanifest') return send(res,200,fs.readFileSync(path.join(ROOT,'public','manifest.webmanifest')),'application/manifest+json; charset=utf-8');
  if(method==='GET' && u.pathname==='/sw.js') { res.writeHead(200,{'Content-Type':'application/javascript; charset=utf-8','Cache-Control':'no-cache','Service-Worker-Allowed':'/','X-Content-Type-Options':'nosniff'});return res.end(fs.readFileSync(path.join(ROOT,'public','sw.js'))); }
