...
- Latest verified deployment is `2b68a6f`; Actions run `36084005412` passed and live `app.css` contains the History width fix. Owner subsequently reported History fits on an iPhone 15 Pro Max running iOS 27.0. The PWA manifest serves at `/static/manifest.webmanifest`; root-scope offline worker and reminders are local candidates, not deployed.
...
| 3 | P2 | Offline Home Screen support — public offline landing implemented locally; release and iPhone check pending | Sol | Root `/sw.js` avoids Azure's stripped worker-scope header and caches only the public offline page/CSS/icon. Confirm Home Screen launches that page without network and returns to live account after reconnect; private records and workout edits remain online-only. Do not cache private API responses. `server.js`, `public/sw.js`, `public/offline.html`, `pwa.test.js`. |
| 4 | P2 | Gentle reminders — in-app and iPhone push code local; VAPID/deployment/device delivery pending | Sol | Owner chose both. In-app notice appears only while open after opt-in and saved local time, and disappears after check-in/dismissal. Push uses an explicit iPhone permission tap, authenticated Apple subscription storage, generic payload, single-instance scheduler and saved-day deduplication. Configure VAPID securely, verify Azure Always On, deploy with approval, then confirm actual iPhone delivery/revocation. `server.js`, `push-reminders.js`, `public/push-settings.js`, `public/reminders.js`. |
