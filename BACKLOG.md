...
- Latest deployed application code is `97ee25e`; Actions run `36075358573` passed and the live root, app JavaScript, favicon and status endpoint returned HTTP 200. The live app JavaScript contains the whole-workout button and iPhone install guide. This verifies publication of those public assets; signed-in iPhone behavior remains for the owner to check. The PWA manifest serves at `/static/manifest.webmanifest`; Azure strips `Service-Worker-Allowed`, so offline service-worker support remains deferred.
...
| --- | --- | --- | --- | --- |
| 1 | P1 | Owner iPhone Safari verification — in progress; History overflow fix local, real-session checks deferred | Owner, with Sol triage of reported failures | Owner reported pass for sign-in/refresh/logout, whole-workout and exercise choices, online Home Screen launch, image upload/preview/caption/replace/delete, CSV/print, and Share create/view/revoke. Owner reported whole-page History overflow. CSS containment fix is local; recheck on iPhone after authorized release. Real workout logging/resume and rest timer await an actual session. Record iOS/Safari versions if available. |
...
| 3 | P2 | Offline Home Screen support — blocked by Azure worker scope | Astra platform review; Sol implementation after approved path | Home Screen metadata, iPhone guide and favicon are live. Offline caching needs an approved public worker path or platform change; private data must never be cached. Do not change EasyAuth access on a guess. `public/index.html`, `public/app.js`, manifest/worker tests. |
| 4 | P2 | Gentle reminders — deferred pending delivery choice | Sol after owner chooses delivery | Define a reliable iOS-compatible delivery route and secure scheduler first. Reminders must be opt-in, timezone-aware, pauseable and free of health details. Do not claim foreground timers work in the background. `server.js`, `public/app.js`. |
