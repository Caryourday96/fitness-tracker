...
test('private session, workout conflicts and reconnect recovery',async()=>{
 ...
 const shiftedDay=offset=>{const date=new Date(`${utcToday}T12:00:00Z`);date.setUTCDate(date.getUTCDate()+offset);return date.toISOString().slice(0,10)};
 const recordedDay=shiftedDay(-2),priorDay=shiftedDay(-3),futureDay=shiftedDay(1);
 ...
 assert.equal((await request('/api/uploads',{data:'data:image/png;base64,'+pixel,day:recordedDay,kind:'public'})).status,400);
 const uploaded=await request('/api/uploads',{data:'data:image/png;base64,'+pixel,day:recordedDay,caption:'Private test'});assert.equal(uploaded.status,201);const image=await uploaded.json();
 assert.equal((await request('/api/uploads/'+image.id,undefined,'GET')).status,200);
 assert.equal((await request('/api/uploads/'+image.id,{caption:'Updated caption',day:priorDay,kind:'progress-photo'},'PUT')).status,200);
 const uploadedList=(await (await request('/api/uploads',undefined,'GET')).json()).uploads[0];assert.equal(uploadedList.caption,'Updated caption');assert.equal(uploadedList.kind,'progress-photo');assert.equal(uploadedList.day,priorDay);
 assert.equal((await request('/api/uploads/'+image.id,{caption:'Invalid type',day:priorDay,kind:'public'},'PUT')).status,400);
 assert.equal((await request('/api/uploads',{data:'data:image/png;base64,AAAA',day:recordedDay})).status,400);
